...
该面板不读取 NovixLink 账户，也不需要商家 API。VPS 上的统计服务持续记录
...
本地统计可能与商家计费存在少量差异，应以 NovixLink 控制面板为最终依据。
...
```bash
RESET_DAY=12 TOTAL_GB=1024 bash <(curl -fsSL https://raw.githubusercontent.com/succichang/surge-bandwagon-traffic/main/install-novixlink-traffic-agent.sh)
```

示例中的含义：

- `RESET_DAY=12`：每月 12 日 UTC 重置。
- `TOTAL_GB=1024`：每月总流量为 1024GB。
...
```bash
systemctl status novixlink-traffic --no-pager
